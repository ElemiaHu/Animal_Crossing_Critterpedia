...
const Fish = () => {
    ...
    const [priceCJ, setPriceCJ] = useState();
    const [hemisphere, setHemisphere] = useState("northern");

    const [allFish, setAllFish] = useState();

    // Defines whether the certain fish is active in a certain month based on hemisphere
    ...
    // Defines whether the certain fish is active in a certain hour
    ...
    const getFish = async () => {
        ...
        setPriceCJ(result['price-cj']);
        setShadow(result.shadow);

        const responseAllFish = await fetch(`https://acnhapi.com/v1/fish`)
        const resultAllFish = await responseAllFish.json()
        setAllFish(resultAllFish);

    }

    // put all fish info into an array
    const fishArray = [];
    for (const property in allFish) {
        fishArray.push(allFish[property]);
    }
    ...
    return <>
        <div className="detailPage">
            ...
            <div className="facts">
                ...
                <div id="price"><span className="property">Price</span>{price} ({priceCJ} when sell to C.J.)</div>
            </div>
        </div>
        ...
        <div className="collection">
            ...
            fishArray.map((fish, index) => (
                <div key={index}><img src={fish['icon_uri']} alt="icon of fish" onClick={() => {setID(index + 1)}}/></div>
            ))
            }
        </div>
    </>
